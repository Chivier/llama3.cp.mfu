...
def counter_update(name, number):
    if name not in flop_counter:
        flop_counter[name] = number
    else:
        flop_counter[name] += number
...
class Array(cp.ndarray, Generic[Shape]): ...
...
def apply_rotary_emb(
    ...
    freqs_cos: Array["B, L or 1, 1, HD//2"] = cp.expand_dims(freqs_cos, axis=(0, 2))
    freqs_sin: Array["B, L or 1, 1, HD//2"] = cp.expand_dims(freqs_sin, axis=(0, 2))
    ...
    counter_update("fmul", xq_r.size + xq_i.size)
    counter_update("fmul", xq_r.size + xq_i.size)
    counter_update("fmul", xk_r.size + xk_i.size)
    counter_update("fmul", xk_r.size + xk_i.size)
    counter_update("fadd", xq_r.size)
    counter_update("fadd", xq_r.size)
    counter_update("fadd", xk_r.size)
    counter_update("fadd", xk_i.size)

    xq_out: Array["B, L or 1, QHN,  HD//2, 2"] = cp.stack([xq_out_r, xq_out_i], axis=-1)
    xk_out: Array["B, L or 1, KVHN, HD//2, 2"] = cp.stack([xk_out_r, xk_out_i], axis=-1)
...
def repeat_kv(x: Array["B, L, KVHN, HD"], n_rep: int):
    if n_rep == 1:
        return x
    z: Array["B, L, QHN, HD"] = cp.repeat(x, n_rep, axis=2)
    return z
...
class FeedForward:
    ...
    def __call__(self, x: Array["B, L or 1, D"]):
        # FD = 2 * 4 * D / 3
        swish: Array["B, L or 1, FD"] = silu(
            cp.asarray(x) @ cp.asarray(self.gate_weight)
        )
        # mult
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.gate_weight.shape[1])
        # silu
        item_count = x.shape[0] * x.shape[1] * self.gate_weight.shape[1]
        counter_update("fdiv", item_count)
        counter_update("fexp", item_count)
        counter_update("fadd", item_count * 2)
        x_V: Array["B, L or 1, FD"] = cp.asarray(x) @ cp.asarray(self.up_weight)
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.up_weight.shape[1])
        x: Array["B, L or 1, FD"] = swish * x_V
        counter_update("fmul", swish.shape[0] * swish.shape[1] * swish.shape[2])
        x: Array["B, L or 1, D"] = cp.asarray(x) @ cp.asarray(self.down_weight)
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.down_weight.shape[1])
        return x
...
class RMSNorm:
    ...
    def __call__(self, x: Array["B, L or 1, D"]):
        ...
        # ! Check
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2])
        z: Array["B, L or 1, D"] = cp.asarray(x) / cp.sqrt(cp.asarray(z))
        counter_update("fdiv", x.shape[0] * x.shape[1] * x.shape[2])
...
class Attention:
    ...
    def __call__(
        ...
        # QKV
        xq: Array["B, L or 1, D"] = cp.asarray(x) @ cp.asarray(self.q_weight)
        xk: Array["B, L or 1, D"] = cp.asarray(x) @ cp.asarray(self.k_weight)
        xv: Array["B, L or 1, D"] = cp.asarray(x) @ cp.asarray(self.v_weight)
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.q_weight.shape[1])
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.k_weight.shape[1])
        counter_update("fmul", x.shape[0] * x.shape[1] * x.shape[2] * self.v_weight.shape[1])
        ...
        counter_update("fmul", xq.shape[0] * xq.shape[1] * xq.shape[2] * xq.shape[3] * xk.shape[3])
        # `mask` is used only once at the beginning.
        if mask is not None:
            attention = attention + mask[None, None, :, :]
        attention = softmax(attention)
        output: Array["B, HN, L or 1, HD"] = attention @ xv
        counter_update("fmul", attention.shape[0] * attention.shape[1] * attention.shape[2] * attention.shape[3] * xv.shape[3])

        # ["B, HN, L or 1, HD"] -> ["B, L or 1, D"]
        output: Array["B, L or 1, D"] = output.transpose(0, 2, 1, 3).reshape(B, L, -1)
        output: Array["B, L or 1, D"] = cp.asarray(output) @ cp.asarray(self.o_weight)
        counter_update("fmul", output.shape[0] * output.shape[1] * output.shape[2] * self.o_weight.shape[1])
...
class Llama:
    def __init__(self, model_path: str, args: ModelArgs):
        ...
        t: Array["M"] = cp.arange(args.max_seq_len)
        freqs: Array["M, HD//2"] = cp.outer(t, inv_freq)
        self.freqs_cos: Array["M, HD//2"] = cp.cos(freqs)
        self.freqs_sin: Array["M, HD//2"] = cp.sin(freqs)
    ...
    def __call__(self, input_ids: Array["B, L"], start_pos: int):
        _, L = input_ids.shape
        h: Array["B, L or 1, D"] = self.tok_embedding[input_ids.get()]
        # ["M, HD//2"] -> ["L or 1, HD//2"]
        ...
        # ["B, 1, VS"] = ["B, 1(L), D"] @ ["D, VS"]
        logit: Array["B, 1, VS"] = cp.asarray(h[:, [-1], :]) @ cp.asarray(
            self.lm_head_weight
        )
        return logit
...
if __name__ == "__main__":
    ...
    print(f"FLOP count: {flop_counter}")
